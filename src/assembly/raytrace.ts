...
function radiance(r: Ray, depth: int, f: Vec, locals: Locals): Vec {
  ...
  var nl: Vec =
    n.dot(r.d) < 0 ? n.clone(locals.loc4) : n.multScalar2(-1, locals.loc4)
  if (!f) {
    f = locals._f
  }
  f.setFrom(obj.c)
  ...
  var n1 = n.multScalar2(
    (into ? 1 : -1) * (ddn * nnt + sqrt<float>(cos2t)),
    locals.loc8
  )
  var tdir: Vec = r.d
      ...
      for (let sy: int = 0; sy < 2; sy++) {
        ...
        for (let sx: int = 0; sx < 2; sx++) {
          ...
          for (let s: int = 0; s < samps; s++) {
            ...
            let dx =
              r1 < 1.0 ? sqrt<float>(r1) - 1.0 : 1.0 - sqrt<float>(2.0 - r1)
            let r2: float = 2.0 * <float>rand()
            let dy =
              r2 < 1.0 ? sqrt<float>(r2) - 1.0 : 1.0 - sqrt<float>(2.0 - r2)
